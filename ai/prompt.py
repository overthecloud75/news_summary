...
SECURITY_REPORT = '''보안 위협 동향 보고서 : [제목 입력]
...
관련 통계나 데이터가 있다면 함께 명시

3. 공격 기술 분석
각 위협 유형에 대해 사용된 공격 기법과 절차를 다음 형식으로 정리하세요:

공격 단계: 초기 침투, 권한 상승, 내부 확산 등

사용된 기법: MITRE ATT&CK 기반으로 설명 (예: T1566.001 - Spearphishing Attachment)

공격 도구 또는 악성코드가 명시된 경우 함께 설명
...
5. 탐지 방안
각 위협 유형에 대해 아래 보안 장비 또는 로그 기반에서 탐지할 수 있는 방법을 항목별로 정리하세요.
각 장비마다 탐지 포인트 또는 룰 조건을 기술하세요:
...
문서에 제시된 위협에 대해, 상황에 적용 가능한 구체적인 예방 및 대응 방안을 설명하세요.
...
각 주요 주제 또는 기술 요소를 항목별로 나누고, 그에 대한 기술적 배경과 상세 설명을 함께 기술하세요.

예시 형식:

[기술 요소 또는 개념명]

기술적 배경: 관련 기술이 어떻게 동작하는지 설명

문서에 등장한 방식: 해당 기술/요소가 문서에서 어떻게 사용되거나 언급되었는지

관련된 맥락: 문서 주제와 어떤 연관이 있는지 간단히 분석
